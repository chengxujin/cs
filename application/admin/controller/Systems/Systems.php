...
//		tpCache($param['inc_type'],$param); //注释掉，不注释会出现重复写入数据库
    ...
    public function app_management(){
        if(IS_POST){
            ...
            if(!$data['android_link'])$this->error("安装包上传失败",U('Systems.Systems/app_management',array('id'=>$data['id'])));
    ...
    public function files_upload(){
        ...
        if($_FILES["android_link"]["error"]) {
           return $_FILES["android_link"]["error"];
        }else {

            //控制上传的文件类型，大小
            if($_FILES["android_link"]["type"]=="application/vnd.android.package-archive" && $_FILES["android_link"]["size"]<1024000)
            {
